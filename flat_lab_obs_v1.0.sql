use etl;
drop procedure if exists generate_flat_lab_obs;
 DELIMITER $$
...
# 1040 = HIV Rapid Test
# 1271 = TESTS ORDERED
# 9239 = LABORATORY TEST WITH EXCEPTION  #Not Applicable in Faces Rugut
# 9020 = LAB ERROR #Not Applicable in Faces Rugut
...
select @start := now();
select @last_date_created_enc := (select max(date_created) from openmrs.encounter);
select @last_date_created_obs := (select max(date_created) from openmrs.obs);
...
select @last_update :=
	if(@last_update is null,
		(select max(date_created) from openmrs.encounter e join flat_lab_obs using (encounter_id)),
		@last_update);
...
(select distinct person_id, obs_datetime
	...
	where voided=1
		...
		and concept_id in (856,5497,730,21,653,790,12,1030,1040,1271,9508, 6126, 887, 6252, 1537, 857)
...
(select
	...
	from voided_obs v
		join openmrs.obs o using (person_id, obs_datetime)
	where
		o.concept_id in (856,5497, 730,21,653,790,12,1030,1040,1271,9508, 6126, 887, 6252, 1537, 857)
...
(select
	...
	from openmrs.obs o use index (date_created)
	where voided=0
