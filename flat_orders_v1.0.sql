use etl;
drop procedure if exists generate_flat_orders;
 DELIMITER $$
...
select @last_date_created := (select max(date_created) from openmrs.orders);
...
select @last_update := '1900-01-01';-- (select max(date_updated) from flat_log where table_name=@table_version);
...
select @last_update :=
	if(@last_update is null,
		(select max(date_created) from openmrs.encounter e join flat_orders using (encounter_id)),
		@last_update);
...
(select patient_id, encounter_id, order_id, date_voided, concept_id, date_created
from openmrs.orders where voided=1 and date_voided > @last_update and date_created > @last_update);
...
(select
	...
	group_concat(concat(@boundary,o.concept_id,'=',o.date_created,@boundary) order by o.concept_id separator ' ## ') as order_datetimes,
	max(o.date_created) as max_date_created
	from voided_orders v
		join openmrs.orders o using (encounter_id)
		left join openmrs.encounter e using (encounter_id)
	where
...
(select
	...
	group_concat(concat(@boundary,o.concept_id,'=',date(o.date_created),@boundary) order by o.concept_id separator ' ## ') as order_datetimes,
	max(o.date_created) as max_date_created

	from openmrs.orders o
		join openmrs.encounter e using (encounter_id)
...
delete t1
from flat_orders t1
join openmrs.person t2 using (person_id)
where t2.voided=1;
