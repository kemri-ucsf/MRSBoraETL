...
select @table_version := "flat_obs_v1.0";
select @start := now();
...
select @last_update :=
	if(@last_update is null,
		(select max(date_created) from openmrs.encounter e join flat_obs using (encounter_id)),
		@last_update);
...
from openmrs.obs where voided=1 and date_voided > @last_update and date_created <= @last_update);
...
delete t1
from voided_obs t1
join openmrs.person_attribute t2 using (person_id)
